...
-- ── Familia (member/family-*.ts) ──────────────────────────────────────────
-- Un integrante por fila. access_level='full' chatea directo con el bot
-- (telegram_chat_id se llena al conectar su invitación); 'managed' es un
-- menor cuyo perfil llevan los de acceso completo.
CREATE TABLE IF NOT EXISTS family_members (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  role TEXT NOT NULL,
  access_level TEXT NOT NULL DEFAULT 'managed',
...
-- kind: 'diaria' (se muestra pendiente de nuevo cada día, aunque se marcó
-- hecha ayer) | 'puntual' (una sola vez). category: 'tarea' | 'ejercicio'.
